...
port_sds <- unname(sapply(seq(0, 1, by = 0.10), port_sd))
  ...
  mutate(
    best_stv_gp = {
      ...
      stvs <- sapply(constant_names, function(gp_name) {
        ...
        sd_port <- unname(port_sds[gp_name])
        p_success / sd_port
      })
...
When comparing Figure 6 to Figure 4, the choice of optimal glide path is much more distributed and less emphasis is placed on purely aggressive portfolios that exhibit potentially unsavory amounts of volatility.
...
stv_counts2 <- stv_df %>%
  count(best_stv_gp) %>%
  transmute(glidepath = best_stv_gp, n_stv = n)
...
cat(sprintf("%-20s %10s %8s %8s %8s %8s\n", "Glide Path", "Port StDev", "# Prob", "% Prob", "# STV", "% STV"))
cat(paste(rep("-", 75), collapse = ""), "\n")
...
In the paper, the STV ratio shifts the optimal portfolio dramatically toward conservative allocations, with 10/90 and 20/80 dominating the majority of scenarios and 100/0 almost never appearing as optimal. Our results show the same qualitative pattern: 100/0 drops from 47% of scenarios under pure probability to just 2% under STV, and 10/90 rises to the most frequently optimal at 20%, confirming that adjusting for volatility strongly penalizes equity heavy portfolios.

# Probability of Ruin Heatmaps

Heat-maps of P(failure) across all 1,071 scenarios for four representative constant portfolios: 0/100 (all bonds), 40/60, 60/40, and 100/0 (all equity).

```{r ruin_heatmaps, fig.height=6}
# Show heat-maps for 4 key portfolios
...
From this plot, it would follow that having better information on how long the distribution period will last would improve the choice of equity allocation leading to favorable bequest results. In the personalized retirement plan section, a P(Death) CDF derived from a Cox Proportional Hazard (PH) model is incorporated with a questionnaire of income, savings, and retirement questions to determine an appropriate glide path. 
...
This section extends Blanchett's framework with your inputs. It uses a personalized mortality CDF (from a Cox PH survival model) combined with a financial situation to discover the optimal lifetime equity glide path.
